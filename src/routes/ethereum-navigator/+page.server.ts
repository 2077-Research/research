...
const isValidPost = (post: BeehiivPost): boolean => {
	return (
		...
		!EXCLUDED_TITLES.some((title) => post.title.toLowerCase().includes(title.toLowerCase()))
	);
};
...
export const load: PageServerLoad = async () => {
	try {
		...
		if (!response.ok) {
			...
			throw error(response.status, errorData.errors?.[0]?.message || 'Failed to fetch posts');
		}

		const data: BeehiivResponse = await response.json();

		const filteredAndSortedPosts = data.data.filter(isValidPost).sort(comparePosts);
